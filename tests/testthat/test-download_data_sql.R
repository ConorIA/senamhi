...
context("Test `download_data_sql()`")

## test senamhiR download (H, CON)
test_that("download_data_sql() can download data", {
  out <- download_data_sql("230715")
...
test_that("download_data_sql() can filter by year", {
...
test_that("download_data_sql() can download DAV stations", {
...
test_that("download_data_sql() can pad with zeroes", {
  out <- senamhiR(401)
...
test_that("download_data_sql() fails when an incorrect station is requested", {
  expect_error(download_data_sql("foo"), "Station ID appears invalid.", fixed=TRUE)
})
