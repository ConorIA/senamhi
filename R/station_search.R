...
station_search <- function(name = NULL, region = NULL, baseline = NULL, ignore.case = TRUE, target = NULL, mindist = 0, maxdist = 100, sort = TRUE, ...) {
  ...
  if (!is.null(baseline)) {
    ...
    for (i in 1:nrow(df)) {
      if (is.na(df$`Data Start`[i]) | df$`Data Start`[i] > baseline[1]) index <- c(index, i)
      else if (is.na(df$`Data End`[i]) | df$`Data End`[i] < baseline[length(baseline)]) index <- c(index, i)
    }
  ...
  if (!is.null(target)) {
    if (length(target) == 1L) {
      p1 <- c(df$Longitude[grep(paste0("\\b", as.character(target), "\\b"), df$StationID)], df$Latitude[grep(paste0("\\b", as.character(target), "\\b"), df$StationID)])
    }
