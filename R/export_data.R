...
export_data <- function(station, year, month = 1:12, write_mode = "z", trim = TRUE) {
  ...
  endDate <- paste(year[length(year)],month[length(month)],number_of_days(endDate), sep = "-")
    ...
    if (nrow(table) > 1) {
      ...
      if (nrow(table)-1 != number_of_days(date)) {
        ...
        if (ncol(table) != length(colnames)) {
          ...
          dat[row:(row+number_of_days(date)-1),2:length(dat)] <- subset(table[2:length(table[, 1]), 2:length(table)])
        }
      }
    } 
  ...
  if (write_mode == "append") write.table(dat, filename, append = TRUE, sep = ",", col.names = FALSE,row.names = FALSE)
